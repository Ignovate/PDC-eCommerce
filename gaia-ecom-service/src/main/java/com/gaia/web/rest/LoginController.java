...
public class LoginController {
	...
	public ResponseEntity<LoginRespVm> login(@RequestBody LoginReqVm loginReq) throws GaiaException {
		...
		return new ResponseEntity<LoginRespVm>(new LoginRespVm(Constants.VALIDATED,Constants.VALIDATED_MSG), HttpStatus.OK);
	}
	
	@PostMapping("signup")
	public ResponseEntity<LoginRespVm> signup(@RequestBody RegisterUserReqVm regUser) throws GaiaException {
		userService.signup(regUser);
		return new ResponseEntity<LoginRespVm>(new LoginRespVm(Constants.VALIDATED,Constants.REGISTERED_MSG), HttpStatus.OK);
